...
class video_controller(BaseVideoController):
    ...
    def show_agents(self, ego_id, agent_list, min_frame, max_frame):
        """
        顯示ego與agent_list於[min_frame, max_frame]範圍
        """
        self.current_ego_id = ego_id
        agent_list = [aid for aid in agent_list if aid != ego_id]
        self.current_agent_ids = agent_list
        self.current_min_frame = min_frame
        self.current_max_frame = max_frame

        frame_set = set()
        if str(ego_id) in self.track_dict:
            frame_set.update(self.track_dict[str(ego_id)].keys())
        for aid in agent_list:
            if str(aid) in self.track_dict:
                frame_set.update(self.track_dict[str(aid)].keys())
        overlay_frames = [f for f in frame_set if int(min_frame) <= int(f) <= int(max_frame)]
        overlay_frames = sorted(overlay_frames, key=lambda x: int(x))

        self.overlay_frame_list = overlay_frames
        self.total_frame_count = len(self.overlay_frame_list)
        ...
        self.range_slider.setMaximum(self.total_frame_count - 1)
        self.range_slider.setValue((0, self.total_frame_count - 1))
        # 不重設 current_frame_no，不重設 slider
        frame = self.image_background.copy()
        self._onscreen_render_cache = {}
        self._update_label_frame(frame)
